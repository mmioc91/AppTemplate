...
## Rules
- Real Postgres via **Testcontainers.PostgreSql** — never `UseInMemoryDatabase` (build error, see root
  `BannedSymbols.txt`), never mock `DbContext`. If optimistic concurrency (e.g. a `xmin`/rowversion token) is
  introduced later, test the actual conflict path, not just the happy path.
